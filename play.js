...
barBuilder(host,cfg){const F=frame(host,cfg);const data=cfg.data||[["Cricket",40],["Football",60],["Tennis",25],["Hockey",35]];let scale=null,val=data.map(()=>0);
  ...
  dragOn(svg,p=>{if(!scale)return null;const bw=300/data.length,i=Math.floor((p.x-L-10+bw*0.2)/bw);return i>=0&&i<data.length?i:null},(i,q)=>{const top=units*scale;val[i]=Math.max(0,Math.min(top,Math.round((B-q.y)/H*top/scale)*scale));draw()});
  F.stage.append(el("p",{class:"ptask"},"Votes for favourite sport. Choose a scale, then drag the top of each bar."),tbl,el("div",{class:"chips"},[1,5,10,50].map(s=>btn("1 unit = "+s,()=>{scale=s;val=data.map(()=>0);draw();
      const fits=Math.max(...data.map(d=>d[1]))<=units*s;F.say(!fits?"Too small – the biggest value ("+Math.max(...data.map(d=>d[1]))+") will not fit on 10 units!":s>=50?"This fits, but the bars will be tiny and hard to compare. Try a smaller scale.":"Good scale – now drag the bars.",fits&&s<50?undefined:false)},"light"))),svg,
    row(btn("Check",()=>{if(!scale)return F.say("Choose a scale first.",false);const bad=data.findIndex((d,i)=>val[i]!==d[1]);F.say(bad<0?F.praise()+" Every bar is the right height. Don’t forget a title and the scale on paper!":data[bad][0]+" should reach "+data[bad][1]+" (that is "+(data[bad][1]/scale)+" units).",bad<0)},"primary")));draw();
},
